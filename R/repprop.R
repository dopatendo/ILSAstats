...
repprop <- function(x,categories = NULL,
                     setup = NULL,
                     repwt = NULL,repindex = NULL,wt,df,
                     method,
  ...
  assignsetup(repprop,setup = setup,mc = match.call())
  ...
  if(!is.null(repindex)){
    ...
    nrep <- lapply(repindex,function(z){

      lapply(z,function(zz){


        outzz <- vector("list",nps)
        for(i in 1:nps){
          outzz[[i]] <- (zz + nrs*(i-1))
    ...
    attributes(nrep) <- attributes(repindex)
  ...
  out <- lapply(1:length(psb), function(i){
    ...
    if(is.null(GR)){
      ...
      frecat <- fri[fri$category%in%1,'frequency']
      ...
      outi[which(!outi$group%in%"Composite"),'N'] <- frecat
    }
  ...
  out <- lapply(out,function(i){
    colnames(i) <- gsub("mean","prop",colnames(i))
    i
  })
